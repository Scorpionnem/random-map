...
execute if score .teambutton settings matches 0 if score .players players matches 2.. run execute at @e[tag=button,type=interaction] run playsound minecraft:ui.button.click ambient @a
execute if score .teambutton settings matches 0 if score .players players matches 2.. run execute at @e[tag=button,type=interaction] run playsound minecraft:block.note_block.banjo ambient @a ~ ~ ~ 1 1.2
...
execute if score .teambutton settings matches 1 as @a[team=!Purple,team=!Lime,tag=player] run scoreboard players add .notinateam players 1
